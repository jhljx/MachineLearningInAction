...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
	...
	while(iters < maxIter):
		...
		for i in range(m):
			fXi = float(np.multiply(alphas, labelMat).T * (dataMatrix * dataMatrix[i,:].T)) + b  #FXi计算预测的类别
			Ei = fXi - float(labelMat[i]) #计算预测结果和真实结果之间的误差
			#如果误差很大，则需要对数据实例对应的alpha值进行优化
			#下面的这个if语句就是判断如果alpha可以更改，进入优化过程
			if ((labelMat[i]*Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
				#if语句中不管是正间隔还是负间隔都会被测试
				#同时检查alpha的值，保证其不能等于0或C
				j = selectJrand(i, m)  #随机选择第二个alpha
				fXj = float(np.multiply(alphas, labelMat).T * (dataMatrix * dataMatrix[j,:].T)) + b
				Ej = fXj - float(labelMat[j]) #同上面的方法计算误差
				alphaIold = alphas[i].copy()
				alphaJold = alphas[j].copy()
				#下面计算L和H，保证alpha在0到C之间
				...
				#检查新的alpha[j]是否有轻微的变化
				if(np.abs(alphas[j] - alphaJold) < 0.00001):
					print 'j not moving enough'
					continue
				#alpha[i]和alpha[j]一起改变，改变大小一样，方向相反(即一个增加，一个减小)
				alphas[i] += labelMat[j] * labelMat[i] * (alphaJold - alphas[j])
				#在对alpha[i]和alpha[j]进行优化之后，给这两个alpha的值设置一个常数项b
				...
				if (0 < alphas[i]) and (C > alphas[i]):
					...
					b= (b1 + b2) / 2.0
				alphaPairsChanged += 1
				print 'iters: %d i:%d, pairs changed %d' % (iters, i, alphaPairsChanged)
